...
import { oldSplitSqlWithPositions, Statement } from './oldSplitSqlWithPositions'
...
async function processError({
    ...
    if (!quoted && innerOffset && innerOffset < sql.length - 1) {
        // channel.appendLine(`error - no quoted, relativeOffset = ${relativeOffset}`)
        const word = sql.substring(innerOffset).match(/\b|$/)
        const length = word?.index ?? sql.length - innerOffset
        diagnostics.push(buildDiagnostic({ document, message, offset: start + innerOffset, length }))
        finish()
        return
    }
    ...
    if (innerOffset && innerOffset < sql.length - quoted[1].length) {
...
export async function oldLintDocument(document: vscode.TextDocument, collection: vscode.DiagnosticCollection) {
    ...
    const statements = await oldSplitSqlWithPositions(document.uri, sqlText)
